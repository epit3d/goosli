...
func TestLine_IntersectLine(t *testing.T) {
	...
	cases := []struct {
		...
		// common
		{
			in1: Line{Point{6, 8, 4}, Point{12, 15, 4}},
			in2: Line{Point{6, 8, 2}, Point{12, 15, 6}},
			out: &Point{9, 11.5, 4},
		},
		// not intersect
		{
			in1: Line{Point{0, 0, 0}, Point{1, 1, 0}},
			in2: Line{Point{1, 0, 0}, Point{2, 1, 0}},
			out: nil,
		},
		// not intersect
		{
			in1: Line{Point{1, 1, 1}, Point{2, 2, 2}},
			in2: Line{Point{9, 0, 9}, Point{0, 9, 9}},
			out: nil,
		},
	...
	for i, row := range cases {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			require.Equal(t, row.out, row.in1.IntersectLine2(2, row.in2))
		})
	}
}
